...
import {
  captureChangeSetBaselineAsync,
  summarizeChangeSetDiffAsync,
  type ChangeSetBaseline,
} from "./changeSetDiff.js";

export type ChangeSetTouchedFileSource = "write" | "edit" | "file_change" | "shell";

export type ChangeSetTouchedFileObservation = {
  path: string;
  source: ChangeSetTouchedFileSource;
};

export type ChangeSetFileAttributionSummary = {
  files: ChangeSetTouchedFileObservation[];
...
type ShellWindow = {
  ...
  rootPath: string;
  ownerKey: string;
  baseline: ChangeSetBaseline | null;
...
function toolSource(chunk: Extract<UiMessageChunk, { type: "tool-input-available" }>): ChangeSetTouchedFileSource | null {
...
export class WorkspaceMutationCoordinator {
  ...
  async beginShell(input: {
    ...
    try {
      window.baseline = await captureChangeSetBaselineAsync(rootPath);
    } catch (error) {
  ...
  async endShell(id: string): Promise<{ paths: string[]; partialReason?: string }> {
    ...
    try {
      if (window.overlapsForeignShell) {
        return {
          paths: [],
          partialReason: "检测到不同 Flow 的 Shell 文件操作时间重叠，相关变化未归属到任何 ChangeSet。",
        };
      }
      if (!window.baseline) {
        return {
      ...
      const summary = await summarizeChangeSetDiffAsync(window.rootPath, window.baseline);
...
export class ChangeSetToolAttributor {
  private readonly activeTools = new Map<string, ActiveTool>();
  private readonly observations = new Map<string, Set<ChangeSetTouchedFileSource>>();
  ...
  async observe(event: RuntimeEvent, chunks: UiMessageChunk[]): Promise<void> {
    for (const chunk of chunks) {
      ...
      if (!source) continue;
      const coordinatorId = `${this.input.agentRunId}:${chunk.toolCallId}`;
  ...
  async finish(): Promise<ChangeSetFileAttributionSummary> {
  ...
  private addObservation(relativePath: string, source: ChangeSetTouchedFileSource) {
    const sources = this.observations.get(relativePath) ?? new Set<ChangeSetTouchedFileSource>();
    sources.add(source);
