...
export function captureChangeSetBaseline(rootPath: string): ChangeSetBaseline {
...
export async function captureChangeSetBaselineAsync(rootPath: string): Promise<ChangeSetBaseline> {
...
export function summarizeChangeSetDiff(rootPath: string, baseline: ChangeSetBaseline): ChangeSetDiffSummary {
  if (baseline.skipped) {
    return { changedFiles: [], text: "", filesChangedSkipped: true };
  }
  fs.mkdirSync(rootPath, { recursive: true });
  const current = captureFileHashes(rootPath);
  const changedFiles: ChangeSetDiffSummary["changedFiles"] = [];
...
export async function summarizeChangeSetDiffAsync(rootPath: string, baseline: ChangeSetBaseline): Promise<ChangeSetDiffSummary> {
  if (baseline.skipped) {
    return { changedFiles: [], text: "", filesChangedSkipped: true };
  }
  await fs.promises.mkdir(rootPath, { recursive: true });
