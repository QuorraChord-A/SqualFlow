...
export default function PlanFeedbackAttachments() {
  const drafts = usePlanFeedbackStore((state) => state.drafts);
  const removeDraft = usePlanFeedbackStore((state) => state.removeDraft);
  const upsertDraft = usePlanFeedbackStore((state) => state.upsertDraft);
  const clearDrafts = usePlanFeedbackStore((state) => state.clearDrafts);
        ...
        {drafts.map((draft) => (
          <Popover key={draft.id} open={editingId === draft.id} onOpenChange={(open) => setEditingId(open ? draft.id : null)}>
            ...
            <PopoverContent side="top" align="start" className="w-[360px] p-3">
              ...
              <div className="flex justify-end gap-2"><Button size="sm" variant="outline" onClick={() => setEditingId(null)}>取消</Button><Button size="sm" disabled={!comment.trim()} onClick={() => { upsertDraft({ flowId: draft.flowId, planRevisionId: draft.planRevisionId, planNodeId: draft.planNodeId, targetLabel: draft.targetLabel, comment: comment.trim() }); setEditingId(null); }}>保存</Button></div>
            </PopoverContent>
          </Popover>
        ))}
      </div>
