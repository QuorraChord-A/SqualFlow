...
export default function OrchestrationPlanPanel({
  ...
  useEffect(() => {
    let stale = false;
    void fetch(`${API_BASE}/api/flows/${flowId}/orchestration-plans`).then((response) => response.ok ? response.json() : []).then((data) => { if (!stale && Array.isArray(data) && data.length > 0) setHistory(data); }).catch(() => {});
    return () => { stale = true; };
  }, [flowId, initialPlan]);
  ...
  const saveFeedback = (node: PlanNodeView | null, comment: string) => upsertDraft({
    flowId,
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-background" data-testid="orchestration-plan-panel">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <div className="mb-3 flex items-center justify-between"><div><h2 className="text-sm font-semibold">全部 Expert 与任务详情</h2><p className="mt-1 text-xs text-muted-foreground">双击任务行或点击铅笔添加评论。</p></div><Badge variant="secondary">{plan.nodes.length} 个任务</Badge></div>
        <div className="grid gap-3">
          {plan.nodes.map((node, index) => {
            const draft = draftFor(node.plan_node_id);
            const selected = selectedNodeId === node.plan_node_id;
            return (
              <article key={node.plan_node_id} ref={(element) => { cardRefs.current[node.plan_node_id] = element; }} onClick={() => setSelectedNodeId(node.plan_node_id)} onDoubleClick={(event) => { if (!isCurrentRevision || (event.target as HTMLElement).closest("button,textarea,input")) return; setAnnotationTarget(node.plan_node_id); }} className={`relative rounded-xl border bg-card/75 p-4 transition-colors ${draft ? "border-primary ring-2 ring-primary/15" : selected ? "border-foreground/35" : "border-border hover:border-foreground/25"}`}>
                ...
                <div className="flex items-start gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-secondary text-xs font-bold">{index + 1}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><Badge variant="outline">{expertLabel(node.expert_id)}</Badge><span className="text-[11px] text-muted-foreground">{node.task?.status ?? "计划中"}</span></div><h3 className="mt-2 text-sm font-semibold">{node.title}</h3></div>
                  {isCurrentRevision ? <AnnotationEditor open={annotationTarget === node.plan_node_id} onOpenChange={(open) => setAnnotationTarget(open ? node.plan_node_id : null)} label={node.title} initialValue={draft?.comment ?? ""} onSave={(comment) => saveFeedback(node, comment)} trigger={<Button type="button" size="icon-sm" variant="ghost" aria-label={`评论此任务：${node.title}`} title="评论此任务"><Pencil /></Button>} /> : null}
                </div>
