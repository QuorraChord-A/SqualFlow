...
function apply(state: TranscriptState, cursor: number, event: TranscriptEvent): TranscriptState {
  return transcriptReducer(state, { type: "apply-events", events: [{ cursor, event }] });
}
...
describe("transcriptReducer guide boundaries", () => {
  it("renders guided user input in SDK order and marks it applied only after assistant output", () => {
    ...
    expect((state.messages[2] as { metadata?: { guideStatusLabel?: string } }).metadata?.guideStatusLabel).toBeUndefined();

    state = apply(state, 6, { type: "text-start", messageId: "msg-assistant-1", id: "text-after" });
    state = apply(state, 7, { type: "text-delta", messageId: "msg-assistant-1", id: "text-after", delta: "引导后输出" });
  ...
  it("creates a new assistant segment after each running guide message", () => {
    ...
    state = apply(state, 6, { type: "text-start", messageId: "msg-assistant-1", id: "text-after-guide-1" });
    state = apply(state, 7, { type: "text-delta", messageId: "msg-assistant-1", id: "text-after-guide-1", delta: "第二段" });
    state = apply(state, 8, {
      type: "message-added",
      message: textMessage("msg-guide-2", "user", "18遍吧", { localMessageKind: "running-guide" }),
    });
    state = apply(state, 9, { type: "text-start", messageId: "msg-assistant-1", id: "text-after-guide-2" });
    state = apply(state, 10, { type: "text-delta", messageId: "msg-assistant-1", id: "text-after-guide-2", delta: "第三段" });
