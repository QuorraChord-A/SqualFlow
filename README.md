...
- **人始终在回路中** — 通过计划/编排审批、用户动作卡和后续消息调整方向；重要决定不被隐藏在后台。
...
3. **审阅计划** — Leader 解释需求计划与多专家编排；在需要时通过对应卡片确认、拒绝或补充约束。
...
| 界面 | Next.js + React：Flow、对话、计划/编排卡、工具与浏览器工作台 |
...
Supervisor 领域模型与 clean-break 协议见 [术语表](docs/supervisor-terminology.md)、[产品契约](docs/supervisor-product-contract.md)、[系统设计](docs/supervisor-architecture.md)、[REST API](docs/supervisor-rest-api.md) 和 [WebSocket 协议](docs/supervisor-ws-protocol.md)。
